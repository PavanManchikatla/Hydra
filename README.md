...
| Coordinator restart resumes the generation (spec §6.5a: derive from the WAL, fence forward) | `hydra-coordinator` | `crates/hydra-node/tests/restart_e2e.rs` — `kill -9` of the real process in three windows (after the first event; mid-stream; INTENT fsynced, COMMIT unsent), restart, `Last-Event-ID` reconnect; three assertions | **✅ product claim — GREEN on 2026-09-03, 3/3 windows** (`hydra-node/tests/restart_e2e.rs`, real arm: `w1_killed_after_the_first_event_resumes_gapless_and_byte_identical ... ok`, `w2_killed_mid_stream_resumes_gapless_and_byte_identical ... ok`, `w3_intent_durable_commit_unsent_fences_forward_and_a_stale_epoch_is_refused ... ok`). Local, engine-gated (CI status *unavailable*). What it cannot see: direct-FWD/D1; a restart that classifies as COMPLETE-but-unfinalized or UNSERVABLE (reported, not driven); more than one crash per session; a pair whose stages acknowledge different frontiers (refused by name, not rebuilt). |
| Loss of the FIRST stage mid-generation is detected by the shipped coordinator and recovered in place (replacement from the same bootstrap; the stream resumes on `Last-Event-ID`) | `hydra-coordinator` + two `hydra-worker` processes | `crates/hydra-node/tests/stage_loss_e2e.rs` — `kill -9` of the real S1 in two windows, replacement from the same `.boot`, three assertions, stale-epoch probe on the survivor | ✅ product, GREEN — 2/2 windows (first stage), real arm, 2026-09-10T16:41Z |
| Loss of the FINAL stage in the two-stage relayed (D0) topology (spec v0.10.5: `BEGIN_RECOVERY` carries `truncate_to = EMPTY`; the survivor discards its KV and the pipeline rebuilds from position 0 at the new epoch) | `hydra-coordinator` + two `hydra-worker` processes | `crates/hydra-node/tests/stage_loss_e2e.rs` — `kill -9` of the real S_P in two windows, replacement from the same bootstrap, three assertions, the durable BEGIN's `truncate_to = -1`, a stale-epoch `APPLY_TOKEN` to the survivor refused | **product ✅ (2026-09-10, spec v0.10.5; 2/2 final-stage windows GREEN on the real arm)** |
...
**Not yet in the product (stated, not implied):** a coordinator restart over a pipeline whose two
frontiers disagree (refused by name — PROJECT_STATE §8); a second stage loss in the same session (reported, not recovered);
