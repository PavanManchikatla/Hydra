...
| 8 | **Placement calibration — the restated 15 % clause** | ⛔ **NOT MET — ESCALATED as a COST-MODEL finding (§7.24)** | **The harness fault is fixed**: `pair::time_teacher_forced_pipeline` opens both connections before the first timer and returns one duration per token, so setup cannot leak in. Measurement now sound — setup excluded by construction, 8 warm-up discarded, **56 steady samples, median**, 64-token prompt. **Predicted 17.61 ms/tok · measured 32.97 · residual +15.37 · error 87.3 %.** **Diagnosed, not just reported** (`fixed_decode_cost.rs`): decode is **not** proportional to layers — full `[0,24)` 16.88 ms/tok vs lower `[0,12)` 10.68 (63.2 % of full for 50 % of layers) and upper 11.23 (66.5 %), so `lower+upper−full = +5.02 ms/tok` of **fixed per-context cost**, paid twice by a 2-stage split. Two missing terms named: **a fixed per-STAGE decode cost** and **a per-CROSSING protocol cost** (encode/decode + BLAKE3 + mTLS over 4 frames/token). **The model was NOT tuned; the assertion is left failing.** Awaiting ratification of the amendment |
...
| Workspace | **250 passed / 0 failed**; clippy clean |
