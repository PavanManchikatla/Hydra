...
| 3 | **GGUF parser fuzzed 24 CPU-hours without crashes** | ⛔ **NOT MET — 8.150 / 24 CPU-hours** | Receipts: `fuzz-32618764167.md`, `fuzz-32618760313.md`, `fuzz-32795673202.md`, `fuzz-32801640792.md`, **`fuzz-32810571619.md`**. 0 crashes across **47.0 billion** cases. **The number is 8.150, and the corrections behind it matter:** leg 3 banks **1.750 not 2.000** because `vendored-gguf` spent 0.250 CPU-hours calling **nothing** while printing `verdict=GREEN` (§7.62). **Fixed, and the fix is observed in CI:** leg 4 logs `target=vendored-gguf … iterations=0 cpu_seconds=0.0 … verdict=UNAVAILABLE` and `FUZZ SUMMARY targets=7 unavailable=1`, so its full 900 s/shard went to real parsers. **⚑ The cadence claim was wrong and is now CLOSED BY OBSERVATION:** no scheduled leg had ever run until **leg 5 (`event=schedule`, 2026-08-25T04:51Z)** — and until the §7.60 fix it would have failed to compile and banked zero while looking normal. **~8 further legs — and now that a SCHEDULED leg has been observed (leg 5, `event=schedule`, run 32810571619), that is also ~8 weeks.** This row is a countdown, not a gap; it cannot be accepted away because the DoD names the number. |
...
* **Row 1 — a scoped disposition.** The owner's one-machine fresh-account run gives a **partial
  ...
  boundary in writing.
* **Row 3 — a countdown.** **~8 more fuzz legs**, at 8.150 / 24. The DoD states the number, so no
