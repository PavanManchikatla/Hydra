...
| 3 | **GGUF parser fuzzed 24 CPU-hours without crashes** | ✅ **MET — 28.150 / 24 CPU-hours, 0 crashes (2026-09-02; legs 8–11 receipted, the first with the VENDORED parser driven — §7.72)** *(2026-09-01: 12.150 after leg 7, `fuzz-33491900881.md`)* | Receipts: `fuzz-32618764167.md`, `fuzz-32618760313.md`, `fuzz-32795673202.md`, `fuzz-32801640792.md`, `fuzz-32810571619.md`, `fuzz-32919303827.md`, **`fuzz-33491900881.md`** (leg 7, the second `event=schedule` leg, 2026-09-01: 8/8 GREEN, 11 973 321 728 cases, 0 crashes). 0 crashes across **71.8 billion** cases. **⚑ Every one of those hours is on the seven pure-Rust parsers: `vendored-gguf` — the parser a worker actually loads through — has 0.000 banked, because `fuzz.yml` never builds the engine (L1 is closed; the cause is now the workflow, not the pin). Directive 2026-09-01 item 4(b) wires it; from that leg on this row's title changes meaning and the receipt says so.** *(Prior text, retained:)* 0 crashes across **59.9 billion** cases. **The number is 10.150, and the corrections behind it matter:** leg 3 banks **1.750 not 2.000** because `vendored-gguf` spent 0.250 CPU-hours calling **nothing** while printing `verdict=GREEN` (§7.62). **Fixed, and the fix is observed in CI:** leg 4 logs `target=vendored-gguf … iterations=0 cpu_seconds=0.0 … verdict=UNAVAILABLE` and `FUZZ SUMMARY targets=7 unavailable=1`, so its full 900 s/shard went to real parsers. **⚑ The cadence claim was wrong and is now CLOSED BY OBSERVATION:** no scheduled leg had ever run until **leg 5 (`event=schedule`, 2026-08-25T04:51Z)** — and until the §7.60 fix it would have failed to compile and banked zero while looking normal. **~7 further legs — and since a SCHEDULED leg has now been observed (leg 5), that is also ~7 weeks.** This row is a countdown, not a gap; it cannot be accepted away because the DoD names the number. |
