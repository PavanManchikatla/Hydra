...
- **Workspace health:** `cargo test --workspace` green (28 tests across `hydra-proto`, `hydra-wal`, `hydra-transport`); spike reproduces from clean build.
...
| **M1** State machines + DST | `hydra-state` pure; `hydra-sim`; I1–I25 as executable checks; mutation parity; TLC suite green | 🔧 **IN PROGRESS** | DoD: 10M+ steps / ≥1,000 seeds zero violations; all directed scenarios; 4 sim mutations caught; TLC green across all 6 configs |
...
| Baseline safety | `BaselineSafety.cfg` | Deepest run to date reached **98M states / 26.6M distinct / depth 143, zero violations** before the session teardown killed the process (no fixpoint yet). Re-running to fixpoint (`verification/run-gate.sh`). Note §9: 8 GB RAM may cap fixpoint — an environment limit, not a model issue; report stats if so |
| Baseline liveness | `BaselineLiveness.cfg` | ⏳ queued behind safety (fairness model completed in v0.10.1: per-(stage, message-class) WF incl. ABORT; coordinator progress actions fair) |
...
| Mut3 (no attempt fencing) | `Mut3AttemptFence.cfg` | ⏳ re-running (prior run was SIGTERM-killed by session teardown at ~2M distinct — **not** a clean drain, so no contingency yet). Expected `ServiceSafety`/`TupleSafety` violation via a stale INITIAL attempt. **Pre-authorized contingency only on a clean drain:** (a) raise `StateConstraint` to `|msgs| ≤ 30`; (b) then `MaxAttempt = 3`; (c) then escalate with stats — never adjust model logic to force it |
...
| TLC: baseline safety fixpoint; baseline liveness; **Mut3 fire-or-contingency** | agent (background) | M1 DoD (d) |
...
Apple Silicon M2, 8 GB RAM (→ small models locally; 70B targets need M2-milestone hardware), macOS, Metal + CPU backends; Rust/cargo; cmake 3.28; flatc 25.12.19; OpenJDK 26 (headless, substituted for temurin cask — approved) + TLC 2.19 (`tla2tools.jar`, git-ignored). Concurrent TLC runs require unique `-metadir` (collision incident logged and fixed).
