...
**Last updated:** 2026-07-12 · **Package version:** v0.10.2 · **Phase:** **M1 in progress** — `hydra-state` complete + `hydra-sim` now drives real stage SMs (all four mutation parities randomized through the sim); gate not yet reached (see §0c) · **Repo:** github.com/PavanManchikatla/Hydra
...
- **Workspace health:** `cargo test --workspace` green (**49 tests** across `hydra-proto`, `hydra-wal`, `hydra-transport`, `hydra-state`, `hydra-sim`; the four mutation feature builds each add their randomized detection test — Mut1/Mut2/Mut3/Mut4 all 200/200); spike reproduces from clean build.
...
| **M1** State machines + DST | `hydra-state` pure; `hydra-sim`; I1–I25 as executable checks; mutation parity; TLC suite green | 🔧 **IN PROGRESS** (`hydra-state` complete; `hydra-sim` drives coordinator + real stage SMs) | Slices 1–5 + sim: activation transaction + I25 + restart; stage SM + F2; recovery Cases A/B/B′/C + RESET; unservable/supersession (I22); ledger + four watermarks + provisional/rollback + emit-after-commit + cancellation + teacher-forcing; invariants I2/I6/I9/I16/I25/F2/CaseBPure/DecisionMonotone. **All 4 mutation parities randomized through the sim (200/200 each): Mut1 med 374, Mut2 med 250, Mut3 med 187, Mut4 med 82.** DoD (remaining): `hydra-sim` 10M+ steps / ≥1,000 seeds zero violations (done locally; CI marathon pending); all directed scenarios; **all 4** sim mutations caught ✅; TLC green across all 6 configs |
