...
- **Workspace health:** `cargo test --workspace` green (**41 tests** across `hydra-proto`, `hydra-wal`, `hydra-transport`, `hydra-state`; +1 under the Mut4 feature build); spike reproduces from clean build.
...
| **M1** State machines + DST | `hydra-state` pure; `hydra-sim`; I1–I25 as executable checks; mutation parity; TLC suite green | 🔧 **IN PROGRESS** (slices 1–4 of ~5) | Slices 1–4: activation transaction + I25 + restart; stage SM + F2; recovery Cases A/B/B′/C + RESET; unservable/supersession (§6.7, I22); invariants I25/I16/I10a/F2/CaseBPure/DecisionMonotone; **all 4 mutation parities** green (Mut1/2/3/4). DoD (remaining): watermarks/ledger/GENERATION_COMMIT; `hydra-sim` 10M+ steps / ≥1,000 seeds zero violations; all directed scenarios; **all 4** sim mutations caught (**Mut1/2/3/4 done**); TLC green across all 6 configs |
