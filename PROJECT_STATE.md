...
- **Active:** M1 in progress (slices 1–3 landed). `hydra-state`: **slice 1** coordinator activation transaction (§6.6) + I25 + §6.5 restart; **slice 2** stage-session SM + F2 attempt fencing; **slice 3** recovery BEGIN_RECOVERY Cases A/B/B′/C + RESET_RECOVERY_ATTEMPT + catch-up/rebuild, with the CaseBPure invariant (I11/I23) and the reset-after-catch-up directed scenario. Pure `(state,event)→(state′,effects[])`, effect-IDs per WAL-FORMAT §4; invariants I25/I16/I10a/F2/CaseBPure. **Three of four mutation parities demonstrated: Mut2 (CaseBPure), Mut3 (fencing), Mut4 (I25)** — checker catches each under its feature. Next: unservable/supersession (Mut1 parity target) → watermarks/ledger/GENERATION_COMMIT → `hydra-sim` (10M-step DST). TLC gate CI-owned (§6).
...
- **Workspace health:** `cargo test --workspace` green (**40 tests** across `hydra-proto`, `hydra-wal`, `hydra-transport`, `hydra-state`; +1 under the Mut4 feature build); spike reproduces from clean build.
...
| **M1** State machines + DST | `hydra-state` pure; `hydra-sim`; I1–I25 as executable checks; mutation parity; TLC suite green | 🔧 **IN PROGRESS** (slices 1–3 of ~5) | Slices 1–3: coordinator activation transaction + I25 + §6.5 restart; stage-session SM + F2 fencing; recovery Cases A/B/B′/C + RESET + catch-up; invariants I25/I16/I10a/F2/CaseBPure; **Mut2 + Mut3 + Mut4 parity** green. DoD (remaining): unservable/supersession (Mut1 parity), watermarks/ledger; `hydra-sim` 10M+ steps / ≥1,000 seeds zero violations; all directed scenarios; **4** sim mutations caught (**Mut2, Mut3, Mut4 done; Mut1 next**); TLC green across all 6 configs |
