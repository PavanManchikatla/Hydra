...
- **Active:** M1 in progress (slices 1–2 landed). `hydra-state`: **slice 1** — coordinator activation transaction (spec §6.6) + I25 abort-finality guard + §6.5 restart; **slice 2** — stage-session SM (`FROZEN_READY→PREACTIVE→ACTIVE_FINAL`, idempotent COMMIT replay, abort→FROZEN_READY) with **F2 attempt fencing**. Pure `(state,event)→(state′,effects[])`, effect-IDs per WAL-FORMAT §4; executable invariants I25/I16/I10a + F2. **Two mutation parities demonstrated: Mut4 (I25) and Mut3 (attempt fencing)** — the checker catches each under its feature flag. Next slices: recovery Cases A/B/B′/C + reset, unservable/supersession, watermarks/ledger, then `hydra-sim` (10M-step DST). TLC gate CI-owned (§6).
- **Blockers:** none. **Awaiting owner decision:** filing the upstream llama.cpp layer-window issue was approved, but the mandatory duplicate search surfaced adjacent open issues (#22436 pipeline-parallelism-over-TCP, #23568 TP+PP) — same problem space, *different ask* (end-user features vs. this low-level API primitive). Per "report back before filing new," **paused** for the owner to choose: file a focused new issue referencing #22436, or comment the proposal onto #22436. Draft is file-ready (`spike/upstream-llama-issue.md`).
- **Workspace health:** `cargo test --workspace` green (**36 tests** across `hydra-proto`, `hydra-wal`, `hydra-transport`, `hydra-state`; +1 under the Mut4 feature build); spike reproduces from clean build.
...
| **M1** State machines + DST | `hydra-state` pure; `hydra-sim`; I1–I25 as executable checks; mutation parity; TLC suite green | 🔧 **IN PROGRESS** (slices 1–2 of ~5) | Slices 1–2: coordinator activation transaction + I25 guard + §6.5 restart; stage-session SM + F2 attempt fencing; effect-IDs; I25/I16/I10a/F2 checks; **Mut3 + Mut4 parity** green. DoD (remaining): stage SM, recovery/reset/unservable/watermarks; `hydra-sim` 10M+ steps / ≥1,000 seeds zero violations; all directed scenarios; **4** sim mutations caught (**Mut3, Mut4 done**); TLC green across all 6 configs |
...
| Upstream layer-window issue: draft **file-ready** (revised generic-first + 2 bug reports). Duplicate search done — related open #22436 (PP-over-TCP), #23568 (TP+PP) are adjacent but distinct. **Paused** ("report back before filing new"): owner picks file-new-referencing-#22436 vs. comment-on-#22436 | owner decision | — |
