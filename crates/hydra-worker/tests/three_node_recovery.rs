...
fn s2_cfg(model: &str, fence: &SessionFence, k1: i32, k2: i32, n_ctx: i32, recovery_start: bool) -> WorkerConfig {
    WorkerConfig { fence: fence.clone(), rank: 1, layer_first: k1, layer_last: k2, is_final: false, receives_tokens: false, epoch: 0, recovery_id: if recovery_start { 1 } else { 0 }, model_path: Some(model.into()), n_gpu_layers: 0, n_ctx, sampler_config: None, recovery_start, shard_manifest: None }
}
fn sp_cfg(model: &str, fence: &SessionFence, k2: i32, n_ctx: i32, recovery_start: bool) -> WorkerConfig {
    WorkerConfig { fence: fence.clone(), rank: 2, layer_first: k2, layer_last: -1, is_final: true, receives_tokens: false, epoch: 0, recovery_id: if recovery_start { 1 } else { 0 }, model_path: Some(model.into()), n_gpu_layers: 0, n_ctx, sampler_config: Some(greedy()), recovery_start, shard_manifest: None }
}

/// A durability target: persist each BOUNDARY_COPY to a real BoundaryStore, ack the fdatasync'd frontier.
fn spawn_durability(cluster: &Cluster, name: &str, path: std::path::PathBuf, fence: SessionFence) -> SocketAddr {
    use std::sync::mpsc;
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async move {
            let listener = hydra_transport::tcp_mtls::TcpMtlsListener::bind_with_config("127.0.0.1:0".parse().unwrap(), server_cfg, hydra_worker::pair::dev_role_table()).await.expect("bind dur");
            ...
            while let Ok(frame) = conn.recv().await {
                if let Ok((view, Msg::BoundaryCopy { boundary_id, first_input_pos, chunk_id, activations, .. })) = wire::decode(&frame.payload, &fence) {
                    ...
                    let d = match store.append_boundary(boundary_id, first_input_pos, chunk_id, &activations) {
...
fn spawn_sink(cluster: &Cluster, name: &str, fence: SessionFence) -> SocketAddr {
    use std::sync::mpsc;
    let id = cluster.issue(name).unwrap();
    let server_cfg = cluster.ca.server_config(&id).unwrap();
    let (tx, rx) = mpsc::channel();
    std::thread::spawn(move || {
        let rt = tokio::runtime::Builder::new_current_thread().enable_all().build().unwrap();
        rt.block_on(async move {
            let listener = hydra_transport::tcp_mtls::TcpMtlsListener::bind_with_config("127.0.0.1:0".parse().unwrap(), server_cfg, hydra_worker::pair::dev_role_table()).await.expect("bind sink");
            tx.send(listener.local_addr().unwrap()).unwrap();
            let mut conn = listener.accept().await.expect("accept").conn;
            while let Ok(frame) = conn.recv().await {
                if let Ok((view, Msg::Fwd { first_input_pos, .. })) = wire::decode(&frame.payload, &fence) {
                    if conn.send(0, &wire::encode_applied_ack(&fence, view.epoch, first_input_pos, &[0u8; 32])).await.is_err() {
                        break;
                    }
                }
            }
...
fn build_pipeline(model: &str, fence: &SessionFence, k1: i32, k2: i32, n_ctx: i32) -> Pipeline {
    let cluster = Cluster::new().unwrap();
    let dir = temp_dir();
    let store1 = dir.join("s1.wal");
    let store2 = dir.join("s2.wal");
    let dur1 = spawn_durability(&cluster, "dur1", store1.clone(), fence.clone());
    let dur2 = spawn_durability(&cluster, "dur2", store2.clone(), fence.clone());

    let sp_id = cluster.issue("sp").unwrap();
    let sp_addr = spawn_multiconn_endpoint(sp_cfg(model, fence, k2, n_ctx, false), cluster.ca.server_config(&sp_id).unwrap());
    ...
    let s2_addr = spawn_multiconn_forwarding_durable_endpoint(
        s2_cfg(model, fence, k1, k2, n_ctx, false), cluster.ca.server_config(&s2_id).unwrap(),
    ...
    let s1_addr = spawn_multiconn_forwarding_durable_endpoint(
        s1_cfg(model, fence, k1, n_ctx), cluster.ca.server_config(&s1_id).unwrap(),
...
async fn three_node_kill_s_p_rebuilds_from_durable_boundaries_and_relinks_byte_identical() {
    ...
    let pl = build_pipeline(&model, &fence, k1, k2, n_ctx);
    let cs_path = pl.dir.join("commit.wal");
    let mut cs = CommitStream::create(&cs_path, CLUSTER_ID, SESSION_ID).unwrap();
    cs.append_initial_commit(&wal_fence(), &admission(&prompt), &initial_checkpoint_bytes(INITIAL_CHECKPOINT_ID, &greedy()), 1).unwrap();
    ...
    let rsp_addr = spawn_multiconn_endpoint(sp_cfg(&pl.model, &fence, k2, n_ctx, true), pl.cluster.ca.server_config(&rsp_id).unwrap());
...
async fn three_node_kill_middle_s2_rebuilds_from_upstream_durable_boundaries_byte_identical() {
    ...
    let sink = spawn_sink(&pl.cluster, "sink", fence.clone());
    ...
    let rs2_addr = spawn_multiconn_forwarding_durable_endpoint(
        s2_cfg(&pl.model, &fence, k1, k2, n_ctx, true), pl.cluster.ca.server_config(&rs2_id).unwrap(),
        ...
        spawn_durability(&pl.cluster, "dur2b", pl.dir.join("s2b.wal"), fence.clone()), "dur2b",
        true, 4,
    );
...
async fn three_node_kill_middle_with_sampled_ahead_survivor_truncates_byte_identical() {
    ...
    let n_ctx = prompt.len() as i32 + n as i32 + 8;
    let fence = SessionFence::dev(0x7B);
    let cfg_hash = greedy().hash();
