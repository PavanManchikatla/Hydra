...
async fn direct_worker_to_worker_fwd_is_bit_exact() {
    ...
    let k = (n_layer / 2).max(1);
    let fence = SessionFence::dev(0xFD);
    let n_ctx = tokens.len() as i32 + 8;
    ...
    let mk = |rank, lf, ll, is_final, toks| WorkerConfig {
        fence: fence.clone(), rank, layer_first: lf, layer_last: ll, is_final,
    ...
    let connector = cluster.coordinator_connector().unwrap();
    let digest = hydra_worker::pair::run_direct_fwd_pipeline(&connector, s1_addr, "worker-s1", &fence, &tokens).await.expect("direct-fwd pipeline");
...
async fn subprocess_worker_survives_kill_9_and_restart() {
    ...
    let fence = SessionFence::dev(0x5b);
    ...
    c.send(0, &wire::encode_commit_activation(&fence, &commit_tuple(), 1)).await.unwrap();
