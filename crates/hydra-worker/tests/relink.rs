...
async fn forward_relinks_to_a_replacement_after_the_target_is_updated() {
    ...
    let fence = SessionFence::dev(21);

    // The replacement downstream (a live control-plane worker).
    let replacement = spawn_endpoint(control_plane_cfg(fence.clone()), ca.server_config(&sp_id).unwrap());
    ...
    assert!(matches!(wire::decode(&resp2, &fence).unwrap().1, Msg::ActivationCommitted(_)));
}
