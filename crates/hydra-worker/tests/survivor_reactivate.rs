...
fn sp_cfg(fence: SessionFence) -> WorkerConfig {
    WorkerConfig {
        fence, rank: 2, layer_first: 0, layer_last: -1, is_final: true, receives_tokens: false,
...
async fn send_recv(conn: &mut ClientConn, fence: &SessionFence, frame: Vec<u8>, label: &str) -> Msg {
    ...
    wire::decode(&f.payload, fence).unwrap_or_else(|e| panic!("decode {label}: {e}")).1
}

async fn activate(conn: &mut ClientConn, fence: &SessionFence, kind: ActivationKind, epoch: u32, rid: u32) {
    let t = tuple(kind, epoch, rid);
    match send_recv(conn, fence, wire::encode_commit_activation(fence, &t, 1), "COMMIT_ACTIVATION").await {
        Msg::ActivationCommitted(_) => {}
        o => panic!("expected ACTIVATION_COMMITTED, got {o:?}"),
    }
    match send_recv(conn, fence, wire::encode_finalize_activation(fence, &t, 1), "FINALIZE_ACTIVATION").await {
        Msg::ActivationFinalized => {}
        o => panic!("expected ACTIVATION_FINALIZED, got {o:?}"),
    }
}

fn setup() -> (SocketAddr, TcpMtls, SessionFence) {
    let ca = ClusterCa::new().unwrap();
    let sp_id = ca.issue(SP).unwrap();
    let coord_id = ca.issue(COORD).unwrap();
    let fence = SessionFence::dev(0x79);
    let addr = spawn_multiconn_endpoint(sp_cfg(fence.clone()), ca.server_config(&sp_id).unwrap());
    let connector = TcpMtls::from_config(ca.client_config(&coord_id).unwrap()).unwrap();
    (addr, connector, fence)
}
...
async fn active_survivor_freeze_reinstall_reactivate_converges() {
    let (addr, connector, fence) = setup();
    ...
    activate(&mut c, &fence, ActivationKind::Initial, 0, 0).await;

    // Freeze (Case A): ACTIVE_FINAL → FROZEN. truncate_to is the durable frontier.
    match send_recv(&mut c, &fence, wire::encode_begin_recovery(&fence, 0, 1, 1, 0), "BEGIN_RECOVERY").await {
        Msg::RecoveryAck { .. } => {}
    ...
    match send_recv(&mut c, &fence, wire::encode_catch_up_context(&fence, 0, 1, 0), "CATCH_UP_CONTEXT").await {
        Msg::CatchUpReady { .. } => {}
    ...
    activate(&mut c, &fence, ActivationKind::Recovery, 1, 1).await;
}
...
async fn active_survivor_reactivation_converges_100x() {
    ...
    for i in 1..=100u32 {
        match send_recv(&mut c, &fence, wire::encode_begin_recovery(&fence, i - 1, i, i, 0), "BEGIN_RECOVERY").await {
            Msg::RecoveryAck { .. } => {}
            o => panic!("iter {i}: expected RECOVERY_ACK, got {o:?}"),
        }
        // Catch up to FROZEN_READY before re-activation (the §7.19 fix — see the diagnostic).
        match send_recv(&mut c, &fence, wire::encode_catch_up_context(&fence, i, i, 0), "CATCH_UP").await {
            Msg::CatchUpReady { .. } => {}
        ...
        activate(&mut c, &fence, ActivationKind::Recovery, i, i).await;
    }
