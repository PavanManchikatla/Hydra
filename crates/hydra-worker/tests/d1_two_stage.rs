...
fn sp_cfg(model: &str, fence: &SessionFence, k: i32, n_ctx: i32, recovery_start: bool) -> WorkerConfig {
    WorkerConfig { fence: fence.clone(), rank: 1, layer_first: k, layer_last: -1, is_final: true, receives_tokens: false, epoch: 0, recovery_id: if recovery_start { 1 } else { 0 }, model_path: Some(model.to_string()), n_gpu_layers: 0, n_ctx, sampler_config: Some(greedy()), recovery_start, shard_manifest: None }
}
fn s1_cfg(model: &str, fence: &SessionFence, k: i32, n_ctx: i32) -> WorkerConfig {
    WorkerConfig { fence: fence.clone(), rank: 0, layer_first: 0, layer_last: k, is_final: false, receives_tokens: true, epoch: 0, recovery_id: 0, model_path: Some(model.to_string()), n_gpu_layers: 0, n_ctx, sampler_config: None, recovery_start: false, shard_manifest: None }
}
...
async fn d1_two_stage_kill_s_p_rebuilds_from_durable_boundaries_byte_identical() {
    ...
    let reference = {
        let s1 = spawn(&cluster, "s1", s1_cfg(&model, &fence, k, n_ctx));
    ...
    c1.send(0, &wire::encode_begin_recovery(&fence, 0, 1, 1, input_pos)).await.unwrap();
    match wire::decode(&c1.recv().await.unwrap().payload, &fence).unwrap().1 {
        Msg::RecoveryAck { .. } => {}
    ...
    let rsp = spawn(&cluster, "sp", sp_cfg(&model, &fence, k, n_ctx, true));
...
async fn d1_two_stage_kill_s1_survivor_sp_frozen_replacement_s1_from_tokens_byte_identical() {
    ...
    let n = 8usize;
    let n_ctx = prompt.len() as i32 + n as i32 + 8;
    let fence = SessionFence::dev(0x15);
    let cfg_hash = greedy().hash();
    let cluster = Cluster::new().unwrap();
    let connector = cluster.coordinator_connector().unwrap();
    ...
    cp.send(0, &wire::encode_begin_recovery(&fence, 0, 1, 1, input_pos)).await.unwrap();
    ...
    let rs1 = spawn(&cluster, "s1r", { let mut c = s1_cfg(&model, &fence, k, n_ctx); c.recovery_id = 1; c.recovery_start = true; c });
