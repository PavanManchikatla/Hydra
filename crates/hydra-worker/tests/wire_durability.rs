...
fn durability_bodies_round_trip_under_the_fence() {
    ...
    let da = wire::encode_durability_ack(&fence, 0, 2, 100, 9);
    match wire::decode(&da, &fence).unwrap().1 {
    ...
    match wire::decode(&wire::encode_commit_ack(&fence, 0, 55), &fence).unwrap().1 {
    ...
    match wire::decode(&wire::encode_commit_sync(&fence, 0, 66), &fence).unwrap().1 {
    ...
    let other_keys = SessionFence::dev(0xB1);
    ...
    let fwd = wire::encode_fwd(&fence, 0, 0, true, &acts);
