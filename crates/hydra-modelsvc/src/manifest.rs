...
impl Manifest {
    ...
    /// Verify the signature against the embedded public key. **This is the gate a worker runs before
    /// trusting any shard** — a failure is a hard refusal, never a warning.
...
impl<'a> Reader<'a> {
    ...
    fn string(&mut self) -> R<String> {
        ...
        String::from_utf8(self.take(n)?.to_vec()).map_err(|_| ManifestError::BadUtf8)
    }
}
...
mod tests {
    ...
    fn sign_verify_roundtrip_and_tamper_detection() {
        ...
        back.verify().expect("round-tripped manifest verifies");
        assert_eq!(back, m);
    ...
    fn unsigned_manifest_refused() {
        assert!(matches!(sample().verify(), Err(ManifestError::Unsigned)));
    }
}
