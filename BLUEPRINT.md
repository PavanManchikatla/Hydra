...
2. **Compute engine:** `llama.cpp`/`ggml` (MIT), vendored as a git submodule, used as a library. Use `ggml-backend` for the device abstraction (CUDA, Metal, CPU in v1; Vulkan later). Study **prima.cpp** (MIT, arXiv:2504.08791) for its pipelined-ring execution and Halda placement solver; port ideas, and code where license-compatible and clean. **Supported model family (v1, amended v0.10.2):** the dense arch families whose per-arch graph builders carry the M−1 layer-window patch — currently **`llama`** (`src/models/llama.cpp`) and **`qwen2`** (`src/models/qwen2.cpp`). Adding a family means porting the ~47-line window patch to that arch's builder and re-running the spike sweep. **The patch is submodule-version-coupled: every `vendor/llama.cpp` bump MUST re-run the M−1 spike sweep (`spike/shard_split`, all split×prompt combinations) as part of the M2 golden-token gate before the bump is accepted.**
   >
   > **⚑ THE ENGINE IS NOW A PROJECT-CONTROLLED FORK (2026-08-25, audit L1 closed).** `vendor/llama.cpp` points at **`https://github.com/PavanManchikatla/llama.cpp.git`**, branch **`hydra-layer-window-f280b26`**, pinned at **`c00bcebf79cbf95be22aa711d485a287914672c5`** — one commit carrying the layer-window patch on top of upstream **`f280b26983ad0fdb705a0d9ebf0503e76f2899b0`** (`ggml-org/llama.cpp`, 2026-08-25). **`spike/llama-cpp-layer-window.patch` is now PROVENANCE, not the mechanism** — a clean checkout gets the patched engine from the pin instead of depending on one machine's uncommitted working tree.
   >
   > **The maintenance cost did not disappear; it became visible.** Syncing to a newer upstream means **re-porting the patch onto the new base and re-running the full sweep before the new SHA is pinned** — the same obligation this clause always stated, now with a named base to port *from*. **Every bump also re-runs the positional-initializer audit on `llama_model_default_params()`**: upstream replaced `use_mmap` / `use_direct_io` / `use_mlock` with the `load_mode` enum inside the last bump's range, and because that initializer is positional with comment labels, a mis-ordered field assigns silently.
