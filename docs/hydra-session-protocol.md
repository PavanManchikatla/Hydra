# Hydra Session Protocol — Specification v0.10.4 (machine-checked transition core)
...
> **⚑ TRUST BOUNDARY (normative; ruled 2026-08-23 after the external security audit).** **v1 assumes every certificate holder is honest.** The fence tuple, mTLS, and the signed manifest establish *which* participant is speaking and *that it is one of ours* — they establish nothing about whether it is telling the truth. A stage holding a valid cluster certificate can misreport a boundary, a capability, or an applied position, and no invariant in this document detects it. Every invariant below is therefore of the form *"given honest participants, the protocol is correct under crash, loss, reorder and partition"* — which is what it was designed for and is the whole of what it claims. **Byzantine-participant resistance is v2 scope.**
...
**Changelog v0.10.3 → v0.10.4** (F-LIVENESS-FAIR, found at the model-checking layer by BaselineLiveness's `Progress` property): §6.4 gains one sentence — exhaustion of activation *or* reconstruction attempts without participant loss resolves via §11 explicit termination (both bounds are policy-set; a bound's outcome is always defined). Two defects were behind the `Progress` counterexample, both in the model, not the protocol: (1) **bound exhaustion** (TLC-3 model-completeness — the finite `MaxAttempt`/`MaxRId` had no terminal outcome; `SessionTerminate`'s guard now covers it); (2) a **model-fidelity gap** — the TLA+ `StageRecvBeginAt` and the Rust stage SM both omitted the §1.3-mandated PREACTIVE-revert arm ("a stage in PREACTIVE receiving `BEGIN_RECOVERY` for the next attempt/epoch treats it per the abort rule: PREACTIVE is reversible"), which marooned a post-supersession stage. **Part (2) is a model/impl correction to *existing* §1.3 text, not a spec change** (the mirror of F-UNSERVABLE: there the prose was wrong and the model right; here the prose is right and the model/impl were incomplete). No new mutation owed (TLC-3 precedent; the fidelity arm is covered by Mut1 continuing to fire).
...
**F2 (control plane).** Epoch/placement/shard/sampler/activation transitions only via precondition-validated messages. **Activation fencing (amended 2026-08-23, audit H1):** a stage accepts an activation control message only if its `activation_attempt_id` lies in the window `{highest_accepted_attempt, highest_accepted_attempt + 1}` for the (session, epoch). Anything **below** is stale; anything **above** is unreachable. Both are rejected (`ERR_FENCED`). On accepting `highest_accepted_attempt + 1` the stage adopts it as its new highest. *The window is sufficient because §6.4 gives the coordinator exactly one way to advance an attempt — `+1` — so a stage that has accepted attempt n can only ever legitimately be offered n (an idempotent replay, which §6.6 step 2 requires it to re-ack) or n+1. It is necessary because a one-sided floor is **unforgeable-past but forgeable-future**: a single message carrying a far-future id is accepted, becomes the new floor, and permanently fences every subsequent legitimate attempt — a silent, permanent denial of activation in which the stage behaves exactly as previously specified.*
...
**Epoch transition (I11):** `BEGIN_RECOVERY{session, base e, target e+1, recovery_id r, old_placement_version, truncate_to}` — Case A (first application at base; freeze; truncate applied state > truncate_to (I7a); discard provisional sampled outputs (I7b); enter target; ack). **Case A admits every stage whose epoch is `base`, whatever its sub-state: ACTIVE_FINAL, FROZEN, and — since §6.5a (2026-09-03) — a stage still REBUILDING or FROZEN_READY at base whose activation never committed because the coordinator crashed first and fenced forward; its unfinished catch-up at the old epoch is abandoned by the same freeze-and-truncate. (Found by the restart oracle's third window: the stage machine dropped the fence-forward `BEGIN_RECOVERY` as Case C and the coordinator waited forever.)** Case B (**pure replay**, to a stage FROZEN/RECOVERING under this exact transition; assert `applied_pos ≤ truncate_to`, fatal on violation — legitimate post-catch-up advancement is handled by `RESET_RECOVERY_ATTEMPT`, never by Case B). Case B′ (a stage in `ACTIVE_FINAL` holding `ACTIVATION_FINALIZED` evidence answers `ERR_RECOVERY_COMPLETED{…, completion_id}` — locally decidable). Case C invalid → `ERR_TRANSITION`.
**Attempt reset (new):**
```text
...
**6.4 Deadlines/attempts (two-level, revised):** every phase has a deadline; a heartbeating stage missing one is reclassified failed. Then: **activation-only failure** (reconstruction intact; e.g., a `COMMIT_ACTIVATION` straggler with shards alive) ⇒ `ACTIVATION_COMMIT_ABORT` (PREACTIVE → FROZEN_READY) and retry activation under `activation_attempt_id + 1`, same `recovery_id`. **Reconstruction-invalidating failure** (a participant's shards/progress lost, or the plan must change) ⇒ `RESET_RECOVERY_ATTEMPT` to all survivors (truncate + FROZEN, `recovery_id + 1`, same base/target epoch), recompute the set, resume from prepare/attach. **A reset does NOT reset the activation-attempt space (amended 2026-08-23, audit M13):** `activation_attempt_id` is monotonic per **(session, epoch)** and is unaffected by `recovery_id` advancing — the coordinator continues from its current attempt, and a stage **retains** its highest accepted attempt across the reset. Only a new epoch begins a fresh attempt space. *Rationale, normative in force: the attempt id exists to fence stale activation traffic within an epoch — §1.1's F2 scopes it to (session, epoch) precisely so a straggler cannot be confused with a live message. Restarting it while the epoch persists would make pre-reset stragglers indistinguishable from post-reset messages, which is the exact confusion `activation_attempt_id` was introduced in v0.9 to remove — and, because the stage floor is epoch-scoped and survives the reset, an implementation that restarted attempts at 0 would have every post-reset activation fenced by the stage's own floor: two conforming implementations, one permanent deadlock.* **Post-decision failure** ⇒ §6.7. Finalized activations are never aborted or reset. **Bound exhaustion (F-LIVENESS-FAIR):** exhaustion of activation attempts *or* reconstruction attempts without participant loss resolves via §11 explicit termination — both bounds are policy-set, and a bound's outcome is always defined (never an indefinite retry/oscillation loop).
...
**6.5a Reconstruction on restart (amended 2026-09-02, design authority; normative).** *The coordinator's volatile state is a pure function of its control WAL and commit stream; a restart resumes no in-flight transaction — it fences forward.* On restart, every on-disk record that passes the WAL's checksum is **durable** (`WalDurable` was only ever an in-process notification; the disk is the truth), and the coordinator's variables are **re-derived**, never assumed to have survived: `activeEpoch` = the maximum epoch over durable `ACTIVATION_COMMIT_INTENT` records (−1 if none); the current recovery's `(target, recovery_id, truncate_to)` = from the latest durable `BEGIN_RECOVERY` record (a `RESET_RECOVERY_ATTEMPT` record advances `recovery_id` at that target); `activation_attempt_id` = the maximum attempt among durable intents at that `(target, recovery_id)` (0 if none); the tuple, prefill and generation watermarks and the installed checkpoint id = from the commit stream (`CommitStream::open`, I19); `completion` (`completion_id`, predecessor) = from the latest durable `ACTIVATION_COMPLETE` if present, else absent; `unservable` = a durable `ACTIVATION_UNSERVABLE` naming **that** completion — the completion at the derived `(target, recovery_id, attempt)`, not any earlier one: a superseded completion of an earlier target is finished business, and the recovery its supersession opened is already in the log as a later `BEGIN_RECOVERY` (found by the DST on 2026-09-03: a restart after supersession's `BEGIN_RECOVERY` was classified SUPERSEDING again, off the old completion, with no completion at the current target). **Then §6.5's classifier runs on the derived values — branches 1–3 (a durable UNSERVABLE, a durable COMPLETE) stand, because they carry completion evidence, *but only while the completed activation has not yet served*: the commit stream is the durable witness of service (a generation commit after the activation), and a coordinator that crashed after serving crashed OUTSIDE any activation transaction — there is no finalization to re-enter, while the stages hold a data-plane tail beyond the durable frontier that only a `BEGIN_RECOVERY`'s truncation removes (§2.3d refuses re-application below a stage's frontier). A served activation therefore fences forward like every other case (refinement 2026-09-03, from the restart oracle's first two windows) — and every other case FENCES FORWARD: the coordinator opens a new recovery at `recovery_id + 1` targeting `epoch + 1` (base = the derived target), and an intent without completion evidence is treated as possibly-delivered and is never resumed.** If the policy bounds on epochs or recovery ids are exhausted, the restart resolves by §11 explicit termination, never by an indefinite crash loop. **Soundness (why derivation is a fence, not a guess):** `ACTIVATION_COMMIT_INTENT` and `BEGIN_RECOVERY` are fsynced **before** they are sent (§6.6 step 1; WAL-before-wire), so the maximum durable value of each is ≥ anything any stage has ever been told — *sent implies durable*. A recovery opened strictly above those maxima therefore outranks every message in flight (F1/F2), and a stage that hears a stale epoch answers `ERR_FENCED`. **Model:** `HydraActivationCore.tla` — `CoordCrash` sets the coordinator's volatile variables to ⊥ and `CoordRestart` derives them from `wal` per this section; the `IntentFence` invariant states the derived attempt is never below a durable intent's, and the `RestartDerivesByMax` mutation (derive the target epoch by minimum instead of maximum) re-opens a recovery below one in flight — a stale `BEGIN_RECOVERY` — which the model's Case B purity (I11/I23) exposes.
